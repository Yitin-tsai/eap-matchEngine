...
public class ReservationReconcilerMetrics {
    ...
    public ReservationReconcilerMetrics(MeterRegistry registry, RedisOrderBookService orderBookService) {
        Gauge.builder("match_reservations_active", orderBookService, RedisOrderBookService::countActiveReservations)
                .description("Current Redis order reservation key count")
                .register(registry);
        this.scannedTotal = Counter.builder("match_reservation_reconciler_scanned_total")
                .description("Total Redis order reservations scanned by MatchEngine reconciliation")
                .register(registry);
        this.completedTotal = Counter.builder("match_reservation_reconciler_completed_total")
                .description("Total Redis order reservations completed by MatchEngine reconciliation")
                .register(registry);
        this.releasedTotal = Counter.builder("match_reservation_reconciler_released_total")
                .description("Total Redis order reservations released by MatchEngine reconciliation")
                .register(registry);
        this.invalidTotal = Counter.builder("match_reservation_reconciler_invalid_total")
                .description("Total invalid Redis order reservations found by MatchEngine reconciliation")
                .register(registry);
        this.failureTotal = Counter.builder("match_reservation_reconciler_failure_total")
